...
function addMethodologySheet(wb, clients, generatedAt) {
  ...
  const stamp = ws.addRow([`Generated: ${generatedAt || new Date().toLocaleString()}`]);
  stamp.font = { italic: true, size: 10, color: { argb: "FF5D5E66" } };
  ws.addRow([]);
 ...
 */
export async function exportExcel(clients, templatePath, generatedAt) {
  ...
  addMethodologySheet(wb, clients, generatedAt);
